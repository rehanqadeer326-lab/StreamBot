...
export class StreamingService {
	...
	private async playVideoFromQueueItem(message: Message | undefined, queueItem: QueueItem): Promise<void> {
	...
	private async executeStream(inputForFfmpeg: any, streamOpts: any, message: Message | undefined, title: string, videoSource: string): Promise<void> {
	...
	private async handleQueueAdvancement(message: Message | undefined): Promise<void> {
		// If loop is enabled, we don't advance the queue, just replay current
		if (this.streamStatus.loop) {
			...
			if (currentItem) {
				...
				setTimeout(() => {
					this.playVideoFromQueueItem(message, currentItem).catch(err =>
						ErrorUtils.handleError(err, 'loop replaying current item')
					);
				}, 1000);
				return;
			}
	...
	private async handleDownload(message: Message | undefined, videoSource: string, title?: string): Promise<string | null> {
	...
	private async prepareVideoSource(message: Message | undefined, videoSource: string, title?: string): Promise<{ inputForFfmpeg: any, tempFilePath: string | null }> {
		const mediaSource = await this.mediaService.resolveMediaSource(videoSource);
	...
	private async executeStreamWorkflow(input: any, options: any, message: Message | undefined, title: string, source: string): Promise<void> {
		this.controller = new AbortController();
		await this.executeStream(input, options, message, title, source);
	}

	private async finalizeStream(message: Message | undefined, tempFile: string | null): Promise<void> {
		if (!this.streamStatus.manualStop && this.controller && !this.controller.signal.aborted) {
	...
	public async playVideo(message: Message | undefined, videoSource: string, title?: string, videoParams?: { width: number, height: number, fps?: number, bitrate?: number }): Promise<void> {
